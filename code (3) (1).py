...
# Usado para armazenar a última imagem gerada para o botão de download
...
# Verifica se as duas condições (API e Uploads) foram atendidas
    ...
    if st.button("Gerar Imagem Única", use_container_width=True, disabled=not ready_to_generate):
        with st.spinner("Gerando sua imagem... ⏳"):
            ...
            if image_bytes:
                ...
                # --- ALTERAÇÃO: Salva a imagem no estado da sessão ---
        ...
        with st.spinner("Gerando a progressão de idade... Isso pode levar um minuto!"):
            ...
            for i, phase in enumerate(phases_to_generate):
                ...
                if image_bytes:
                    ...
                    st.session_state.last_image_info = None # Limpa para não mostrar o botão de salvar
                    progress_bar.empty()
                    break
            ...
            if len(generated_images) == 4:
                ...
                # --- ALTERAÇÃO: Converte a imagem da grade para bytes e salva no estado da sessão ---
                buf = io.BytesIO()
...
# --- NOVO: PASSO 3 - SALVAR IMAGEM ---
# Esta seção só aparece se uma imagem foi gerada com sucesso
